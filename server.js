...
let state = {
  ...
  theme: saved?.theme ? { ...DEFAULT_THEME, ...saved.theme } : { ...DEFAULT_THEME },
...
function loadScheduleItem(index) {
  if (index >= 0 && index < state.schedule.length) {
    ...
    state.lines = item.lines && item.lines.length > 0 ? item.lines : ['(Empty item)'];
...
function saveSchedule(schedule) {
  const existingIdx = state.savedSchedules.findIndex(s => s.id === schedule.id);
  if (existingIdx >= 0) {
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/lyrics/search') {
    ...
    fetch(targetUrl, {
      headers: {
        'User-Agent': 'WorshipPresenterStudio/2.0 (https://github.com/worshippresenter)',
      },
    })
      .then(r => r.json())
      .then(data => {
        const results = (Array.isArray(data) ? data : [])
          ...
          .slice(0, 15)
          .map(item => {
            const raw = item.plainLyrics || (item.syncedLyrics ? item.syncedLyrics.replace(/\[\d+:\d+\.\d+\]\s*/g, '') : '');
      ...
      .catch(err => {
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ results: [], error: 'Failed to query online lyric provider' }));
      });
    return;
  }
  ...
  if (!fs.existsSync(filePath)) {
    ...
    const fallbackPath = path.join(PUBLIC_DIR, pathname === '/index.html' ? 'control.html' : pathname);
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('----------------------------------------------------');
  console.log('  ✨ WorshipPresenter Studio Server is running!');
  console.log('----------------------------------------------------');
  console.log(`  🖥️  Local Control:    http://localhost:${PORT}/`);
  console.log(`  📺 Local Display:    http://localhost:${PORT}/display`);
  console.log(`  🎬 Local vMix Ovly:  http://localhost:${PORT}/display?overlay=1`);
  ...
  if (ips.length > 0) {
    ...
    ips.forEach(ip => {
      ...
      console.log(`     • vMix Overlay:   http://${ip}:${PORT}/display?overlay=1`);
